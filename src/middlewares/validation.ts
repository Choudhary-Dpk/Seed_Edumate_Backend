 ...
 */
export const validateQuery = <T extends z.ZodRawShape>(schema: z.ZodObject<T>) => {
 ...
 */
export const validateParams = <T extends z.ZodRawShape>(schema: z.ZodObject<T>) => {
  return (req: Request, res: Response, next: NextFunction): void => {
    try {
 ...
 */
export const validateBody = <T extends z.ZodRawShape>(schema: z.ZodObject<T>) => {
  return (req: Request, res: Response, next: NextFunction): void => {
    try {
...
export const admissionStatusQuerySchema = z.object({
  status: z.enum(admissionStatusValues, {
    message: 'Status must be one of: Not Applied, Applied, Interview Scheduled, Waitlisted, Admitted, Rejected'
  })
});
...
export const studyDestinationQuerySchema = z.object({
  destination: z.enum(studyDestinationValues, {
    message: 'Destination must be one of: US, UK, Canada, Australia, Germany, France, Singapore, Italy, UAE, Other'
  })
});
...
const educationLevelValues = ['High School', "Bachelor's", "Master's", 'PhD', 'Other'] as const;

// Current education schema
const currentEducationSchema = z.object({
  level: z.enum(educationLevelValues).optional(),
  institution: z.string().max(200).optional(),
  major: z.string().max(200).optional(),
  cgpaPercentage: z.number()
...
// Degree level enum
const degreeLevelValues = ["Bachelor's", "Master's", 'PhD', 'Diploma', 'Certificate', 'Professional Course'] as const;

// Target education schema
const targetEducationSchema = z.object({
  ...
  intendedStartDate: z.date().min(new Date(), 'Intended start date cannot be in the past').optional(),
  intendedStartTerm: z.string().max(50).optional(),
  courseDurationMonths: z.number()
    .int()
...
const statusDispositionValues = [
  'Not Interested', 'Wrong Number', 'Call not Answered', 
  'Follow Up', 'Int for Next Year', 'Partial Documents Received'
] as const;
...
const applicationJourneySchema = z.object({
...
const coApplicantsSchema = z.array(
  z.object({
...
// Collateral schema
const collateralSchema = z.array(
  z.object({
...
// Loan type enum
const loanTypeValues = ['Secured', 'Unsecured', 'Education Loan', 'Personal Loan'] as const;
const repaymentTypeValues = ['EMI', 'Bullet Payment', 'Interest Only'] as const;

// Loan preferences schema
const loanPreferencesSchema = z.object({
...
const leadSourceValues = [
  'Organic Search', 'Social Media', 'B2B Partner', 'Referral', 
  'Advertisement', 'Website', 'Walk-in', 'Other'
] as const;
...
export const advancedSearchSchema = z.object({
  ...
  (data) => {
    ...
    const definedKeys = Object.entries(data).filter(([, value]) => value !== undefined);
...
export type CreateEdumateContactInput = z.infer<typeof createEdumateContactSchema>;
export type UpdateEdumateContactInput = z.infer<typeof updateEdumateContactSchema>;
