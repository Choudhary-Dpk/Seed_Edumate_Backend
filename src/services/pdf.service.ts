...
export interface PDFGenerationOptions {
  ...
  /**
   * IANA timezone string from the requester (e.g. "Asia/Kolkata",
   * "America/New_York"). Frontend can pass
   * `Intl.DateTimeFormat().resolvedOptions().timeZone`. Falls back to
   * "Asia/Kolkata" when omitted.
   */
  timezone?: string;
  /**
   * BCP 47 locale tag (e.g. "en-IN", "en-US"). Controls date/time/number
   * formatting style. Falls back to "en-IN" when omitted.
   */
  locale?: string;
}
...
class BrowserManager {
  ...
  static async getBrowser(): Promise<Browser> {
    ...
    if (
      ...
      logger.debug("Browser idle too long, closing...");
...
process.on("exit", () => {
  logger.debug("Process exiting, cleaning up browser...");
});
...
export const generatePDF = async (
  ...
  try {
    ...
    logger.debug("Error generating PDF:", error);
    ...
    if (page) {
      try {
        ...
        logger.debug("Error closing page:", closeError);
      }
    }
  }
};
