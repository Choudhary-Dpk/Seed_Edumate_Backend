...
import { MappedEdumateContact, PaginationOptions } from "../types";
import {
  createHubSpotError,
  createNotFoundError,
...
export const getEdumateContacts = async (
  ...
  try {
    ...
    logger.info("Retrieved and mapped Edumate contacts", {
...
export const getEdumateContactById = async (
  ...
  try {
    ...
    logger.error("Error in getEdumateContactById service", {
      contactId,
      error,
    });
...
export const createEdumateContact = async (
  contactData: Partial<MappedEdumateContact>
): Promise<MappedEdumateContact> => {
  try {
    ...
    logger.info("Created and mapped Edumate contact", {
      contactId: mappedContact.id,
    });
    return mappedContact;
  } catch (error) {
    logger.error("Error in createEdumateContact service", {
      error,
      contactData,
    });
    throw createHubSpotError(
...
export const updateEdumateContact = async (
  ...
  try {
    ...
    const hubspotContact = await hubspotClient.updateEdumateContact(
      contactId,
      properties
    );
    const mappedContact = HubSpotMapper.mapEdumateContact(hubspotContact);
    ...
    logger.error("Error in updateEdumateContact service", {
      contactId,
      error,
      contactData,
    });
...
export const batchCreateEdumateContacts = async (
  contactsData: Partial<MappedEdumateContact>[]
): Promise<{
  success: number;
  failed: number;
  results: any[];
  ...
  try {
    ...
    const propertiesArray = await Promise.all(
      contactsData.map((contactData) => mapToHubSpotProperties(contactData))
    );

    const response = await hubspotClient.batchCreateEdumateContacts(
      propertiesArray
    );
...
export const batchUpdateEdumateContacts = async (
  ...
  try {
    ...
    const response = await hubspotClient.batchUpdateEdumateContacts(
      formattedUpdates
    );
...
export const searchEdumateContactsByEmail = async (
  email: string
): Promise<MappedEdumateContact[]> => {
  try {
    ...
    logger.info("Searched and mapped Edumate contacts by email", {
      email,
      count: contacts.length,
    });
    return contacts;
  } catch (error) {
    logger.error("Error in searchEdumateContactsByEmail service", {
      email,
      error,
    });
    throw createHubSpotError(
...
export const searchEdumateContactsByPhone = async (
  phoneNumber: string
): Promise<MappedEdumateContact[]> => {
  try {
    const searchRequest = hubspotClient.createSearchRequest(
    ...
    logger.info("Searched and mapped Edumate contacts by phone", {
      phoneNumber,
      count: contacts.length,
    });
    return contacts;
  } catch (error) {
    logger.error("Error in searchEdumateContactsByPhone service", {
      phoneNumber,
      error,
    });
    throw createHubSpotError(
...
export const searchEdumateContactsByAdmissionStatus = async (
  status: string
): Promise<MappedEdumateContact[]> => {
  try {
    const searchRequest = hubspotClient.createSearchRequest(
    ...
    logger.info("Searched and mapped Edumate contacts by admission status", {
      status,
      count: contacts.length,
    });
    return contacts;
  } catch (error) {
    logger.error("Error in searchEdumateContactsByAdmissionStatus service", {
      status,
      error,
    });
    throw createHubSpotError(
...
export const searchEdumateContactsByStudyDestination = async (
  destination: string
): Promise<MappedEdumateContact[]> => {
  try {
    const searchRequest = hubspotClient.createSearchRequest(
    ...
    logger.info("Searched and mapped Edumate contacts by study destination", {
      destination,
      count: contacts.length,
    });
    return contacts;
  } catch (error) {
    logger.error("Error in searchEdumateContactsByStudyDestination service", {
      destination,
      error,
    });
    throw createHubSpotError(
...
const mapToHubSpotProperties = async (
  contactData: Partial<ContactsLead>
): Promise<Record<string, any>> => {
  const properties: Record<string, any> = {};
  const loanAmount =
    contactData?.selectedLoanCurrency != "INR" && contactData?.loanAmount
      ? await convertCurrency(
          Number(contactData?.loanAmount) || 0,
          contactData?.selectedLoanCurrency || "INR",
          "INR"
        )
      : contactData?.loanAmount;
  const coApplicantAnnualIncome =
    contactData?.baseCurrency != "INR" && contactData?.coApplicantAnnualIncome
      ? await convertCurrency(
          Number(contactData?.coApplicantAnnualIncome),
          contactData?.baseCurrency || "INR",
          "INR"
        )
      : contactData?.coApplicantAnnualIncome;
  ...
  if (contactData.phoneNumber)
    properties.phone_number = contactData.phoneNumber;
  if (contactData.dateOfBirth)
    properties.date_of_birth = contactData.dateOfBirth
      .toISOString()
      .split("T")[0];
  if (contactData.gender) properties.gender = contactData.gender;
  if (contactData.selectedLoanCurrency)
    properties.user_selected_currency = contactData.selectedLoanCurrency;
  if (contactData.baseCurrency)
    properties.base_currency = contactData.baseCurrency;
  if (contactData.studyDestinationCurrency)
    properties.study_destination_currency =
      contactData.studyDestinationCurrency;
  ...
  if (contactData.loanPreference)
    properties.loan_type_preference = contactData.loanPreference;
  if (contactData.intakeMonth)
    properties.intake_month = contactData.intakeMonth;
  if (contactData.intakeYear) properties.intake_year = contactData.intakeYear;
  if (contactData?.loanAmount)
    properties.loan_amount_required = loanAmount || contactData?.loanAmount;

  // if (contactData.loanAmount) properties.co_applicant_1_name = contactData.loanAmount;
  if (contactData.coApplicant)
    properties.co_applicant_1_relationship = contactData.coApplicant;
  if (contactData.coApplicantIncomeType)
    properties.co_applicant_1_occupation = contactData.coApplicantIncomeType;
  if (contactData.coApplicantAnnualIncome)
    properties.co_applicant_1_income = coApplicantAnnualIncome;
  if (contactData.coApplicantMobile)
    properties.co_applicant_1_mobile_number = contactData.coApplicantMobile;
  if (contactData.coApplicantEmail)
    properties.co_applicant_1_email = contactData.coApplicantEmail;

  if (contactData?.analyticalExam?.GRE)
    properties.gre_score = contactData?.analyticalExam?.GRE;
  if (contactData?.analyticalExam?.GMAT)
    properties.gmat_score = contactData?.analyticalExam?.GMAT;
  if (contactData?.analyticalExam?.SAT)
    properties.sat_score = contactData?.analyticalExam?.SAT;
  if (contactData?.analyticalExam?.CAT)
    properties.cat_score = contactData?.analyticalExam?.CAT;
  if (contactData?.analyticalExam?.NMAT)
    properties.nmat_score = contactData?.analyticalExam?.NMAT;
  if (contactData?.analyticalExam?.XAT)
    properties.xat_score = contactData?.analyticalExam?.XAT;
  if (contactData?.languageExam?.TOEFL)
    properties.toefl_score = contactData?.languageExam?.TOEFL;
  if (contactData?.languageExam?.IELTS)
    properties.ielts_score = contactData?.languageExam?.IELTS;
  if (contactData?.languageExam?.Duolingo)
    properties.duolingo_score = contactData?.languageExam?.Duolingo;

  if (contactData.utm_campaign)
    properties.utm_campaign = contactData.utm_campaign;
  if (contactData.utm_medium)
    properties.loan_amount_required = contactData.utm_medium;
  if (contactData.utm_source) properties.utm_source = contactData.utm_source;

  if (contactData.targetDegreeLevel)
    properties.target_degree_level = contactData.targetDegreeLevel;
  if (contactData.admissionStatus)
 ...
 */
export const fetchPartnerByEmail = async (email: string): Promise<any> => {
  const searchRequest = hubspotClient.createSearchRequest(
    [
      {
...
export const createEdumateContactsLeads = async (leads: ContactsLead[]) => {
  try {
...
export const mapLeadToHubSpotProperties = (
  lead: ContactsLead
): Record<string, any> => {
  const properties: Record<string, any> = {};
  ...
  if (lead.educationLevel)
    properties.current_education_level = lead.educationLevel;
...
export const createContactsLoanLeads = async (
  leads: ContactsLead[],
  b2bPartnerHSId: string | null = null
) => {
  try {
    const response = await hubspotClient.createMultiContactsLead(
      leads,
      b2bPartnerHSId
    );
