...
const commissionPgClient = new Client({
...
export async function startCommissionPGNotifyWorker() {
  try {
    ...
    await commissionPgClient.connect();
    logger.info("[Commission PG NOTIFY] Connected to PostgreSQL");

    await commissionPgClient.query("LISTEN commission_sync_channel");
    logger.info("[Commission PG NOTIFY] Listening on commission_sync_channel");

    commissionPgClient.on("notification", handleNotification);

    commissionPgClient.on("error", (err: any) => {
      logger.error("[Commission PG NOTIFY] PostgreSQL connection error:", err);
      reconnectcommissionPgClient();
    });
...
async function reconnectcommissionPgClient() {
  logger.warn(
    "[Commission PG NOTIFY] Attempting to reconnect to PostgreSQL..."
  );

  try {
    await commissionPgClient.end();
    await commissionPgClient.connect();
    await commissionPgClient.query("LISTEN commission_sync_channel");
    logger.info("[Commission PG NOTIFY] Reconnected to PostgreSQL");
  } catch (error) {
    logger.error("[Commission PG NOTIFY] Reconnection failed:", error);
    setTimeout(reconnectcommissionPgClient, 5000);
  }
}
