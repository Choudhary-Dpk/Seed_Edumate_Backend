...
async function handleNotification(msg: any) {
    ...
    try {
        ...
        if (data.source === "hubspot" && data.operation === "UPDATE") {
            logger.debug(
                "[B2B Partner PG NOTIFY] Skipping HubSpot-source UPDATE"
            );
            return;
        }
        ...
        if (data.operation === "UPDATE") {
            ...
            const timeout = setTimeout(() => {
                ...
                queueB2BPartnerSync(partnerId, data.operation, data.hs_object_id, syncKey);
            }, DEBOUNCE_DELAY);
        ...
        logger.error("[B2B Partner PG NOTIFY] Failed to handle notification:", error);
    }
}
...
async function queueB2BPartnerSync(
    ...
    b2bPartnerSyncQueue.add(async () => {
        ...
        try {
            ...
            const result = await processB2BPartnerSync(partnerId, operation, hsObjectId);
            ...
            logger.error(`[B2B Partner PG NOTIFY] Sync failed for ${syncKey}:`, error);
...
async function transformToHubSpotFormat(partner: any): Promise<any> {
    ...
    return {
        ...
        student_capacity_monthly: businessCapabilities.student_capacity_monthly || null,
        student_capacity_yearly: businessCapabilities.student_capacity_yearly || null,
        ...
        commission_model: mapEnumValue("commission_model", commissionStructure.commission_model),
        commission_rate: commissionStructure.commission_rate || null,
        commission_type: mapEnumValue("commission_type", commissionStructure.commission_type),
        ...
        payment_frequency: mapEnumValue("payment_frequency", commissionStructure.payment_frequency),
        payment_method: mapEnumValue("payment_method", commissionStructure.payment_method),
        ...
        tds_rate: commissionStructure.tds_rate
            ...
            : null,
        tiered_commission_structure: commissionStructure.tiered_commission_structure || null,
        ...
        primary_contact_designation: contactInfo.primary_contact_designation || null,
        ...
        marketing_materials_provided: marketingPromo.marketing_materials_provided || null,
        ...
        // Partnership Details
        agreement_type: mapEnumValue("agreement_type", partnershipDetails.agreement_type),
        partnership_end_date: partnershipDetails.partnership_end_date
            ? new Date(partnershipDetails.partnership_end_date).toISOString().split("T")[0]
            : null,
        partnership_start_date: partnershipDetails.partnership_start_date
            ? new Date(partnershipDetails.partnership_start_date).toISOString().split("T")[0]
            : null,
        partnership_status: mapEnumValue("partnership_status", partnershipDetails.partnership_status),
        ...
        lead_conversion_rate: performanceMetrics.lead_conversion_rate
            ...
            : null,
        leads_converted_to_applications: performanceMetrics.leads_converted_to_applications || null,
        ...
        partner_rating: performanceMetrics.partner_rating
            ...
            : null,
        qualified_leads_provided: performanceMetrics.qualified_leads_provided || null,
        seasonal_performance_pattern: performanceMetrics.seasonal_performance_pattern || null,
        ...
        assigned_account_manager: relationshipManagement.assigned_account_manager || null,
        ...
        joint_marketing_activities: relationshipManagement.joint_marketing_activities || null,
        last_interaction_date: relationshipManagement.last_interaction_date
            ? new Date(relationshipManagement.last_interaction_date).toISOString().split("T")[0]
            : null,
        ...
        integration_status: mapEnumValue("integration_status", systemTracking.integration_status),
...
async function markAsProcessing(outboxId: string): Promise<any> {
    ...
    logger.debug(`[B2B Partner PG NOTIFY] Marked outbox #${outboxId} as PROCESSING`);
    return updated;
}
