...
async function moveFieldToSection(client, fieldDef, sectionApiName, layoutId, sectionLookup, dryRun) {
  ...
  const currentSection = fieldDef.section?.api_name || fieldDef.section?.display_label || 'unknown';
  ...
  if (dryRun) {
    info(`  [DRY RUN] Would move '${apiName}' from '${currentSection}' → '${targetDisplayLabel}'`);
    return 'would_move';
  }

  // Look up the target section's real api_name (Zoho v8 uses __s suffix
  // for custom sections, e.g. "Personal_Information__s"). The lookup
  // map is keyed by display label and gives us the actual section api_name.
  const targetSection = sectionLookup.get(targetDisplayLabel);
  if (!targetSection) {
    error(`  [FAIL] Section '${targetDisplayLabel}' not found in layout. Cannot move '${apiName}'.`);
    return 'error';
  }

  // Zoho v8: PATCH /settings/fields/{field_id}?module=Contacts
  // Update the field's section reference using the section's real api_name.
  const payload = {
    fields: [{
      api_name: apiName,
      section: { api_name: targetSection.api_name },
    }],
  };

  const url = `/settings/fields/${fieldDef.id}?module=${MODULE}`;
  const res = await client.patch(url, payload);

  if ([200, 201, 202].includes(res.status)) {
    const fieldResults = res.data?.fields || [];
    if (fieldResults[0]?.status === 'success') {
      info(`  [MOVED] '${apiName}' → '${targetDisplayLabel}'`);
      return 'moved';
    } else {
      error(`  [FAIL] Move failed for '${apiName}': ${JSON.stringify(fieldResults)}`);
      await sleep(SLEEP_AFTER_ERROR);
      return 'error';
    }
  } else {
    error(`  [FAIL] HTTP ${res.status} moving '${apiName}': ${JSON.stringify(res.data)}`);
    await sleep(SLEEP_AFTER_ERROR);
    return 'error';
  }
}
...
async function main() {
  ...
  try {
    ...
    // Step 5: Move each existing field into its assigned section
    ...
    for (const [sectionApiName, fieldList] of Object.entries(sectionsToProcess)) {
      ...
      for (const apiName of fieldList) {
        ...
        if (!fieldDef) {
          warn(`  [SKIP] Field '${apiName}' not found in Zoho.`);
          stats.skipped_missing += 1;
          continue;
        }
        const status = await moveFieldToSection(
          client, fieldDef, sectionApiName, layout.id, sectionLookup, dryRun,
        );
        stats[status] = (stats[status] || 0) + 1;
        if (status === 'error') errorFields.push(`${sectionApiName}.${apiName}`);
    ...
    if (errorFields.length > 0) {
      info(`  Failed fields:    ${JSON.stringify(errorFields)}`);
    }
    info('='.repeat(60));
  } catch (err) {
